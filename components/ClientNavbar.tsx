      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center space-x-8">
            <NavLink href="/client" pathname={pathname} label="Dashboard" />
            <NavLink href="/client/pet-types" pathname={pathname} label="Kelola Hewan Peliharaan" />
            <NavLink href="/client/pet_treatments" pathname={pathname} label="Daftar Kunjungan" />
...
function NavLink({ href, pathname, label }: { href: string; pathname: string; label: string }) {
  ...
  return (
    <Link
      href={href}
      className={`flex items-center h-full px-1 ${isActive ? "text-[#FD7E14] border-b-2 border-[#FD7E14]" : "text-gray-900 hover:text-[#FD7E14]"}`}
    >
      {label}
    </Link>
  );
