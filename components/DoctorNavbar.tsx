        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-8 h-10">
            ...
            <div className="relative h-full flex items-center group">
              <Link
                href="/doctor/manage"
                className={`flex items-center h-full px-1 ${pathname.startsWith('/doctor/manage') ? activeClass : inactiveClass}`}
              >
                Kelola Data
                <svg
...
function DropdownLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className="block px-4 py-2 text-sm text-gray-700 hover:bg-[#FFF5EB] hover:text-[#FD7E14] transition-colors"
    >
      {label}
    </Link>
  );
